...
def collect_metric_from_splunk(
    metric_name,
    index_name,
    selector,
    user,
    ...
    if "*" in metric_name:
        for event in events:
            ...
            for metric, value in event.items():
                if metric == selector:
                    continue
                result[f"{name}-{metric[4:-1]}"] = float(value)
    else:
        for event in events:
            result[event[selector]] = float(event[f"{func}({metric_name})"])
    return result
...
def _collect_events(
    query, start_time, end_time, url="", user="", password="", type="events"
):
    """
    ...
    job_id = json_res["sid"]
    events = _wait_for_job_and__get_events(job_id, url, user, password, type)
...
def _wait_for_job_and__get_events(job_id, url="", user="", password="", type="events"):
    """
    ...
    for _ in range(TIMEROUT):
        ...
        if dispatch_state == "DONE":
            events = _get_events(job_id, url, user, password, type)
            break
...
def _get_events(job_id, url="", user="", password="", type="events"):
    """
